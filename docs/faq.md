...
PSRule for CAF provides several baselines that can be referenced when running PSRule.
...
Currently PSRule for CAF supports testing Azure resources from Infrastructure as Code (IaC) with:
...
What is supported?
After resources are deployed to Azure, PSRule for CAF can be used to check the Azure resources **in-flight**.
...
PSRule for CAF supports two methods for analyzing Azure resources:
...
- **Pre-flight** &mdash; Before resources are deployed from an ARM template or Bicep.
  ...
  - Improve alignment of resources to CAF recommendations.
...
PSRule for CAF covers common use cases that align to the [Microsoft Azure Well-Architected Framework][AWAF].
...
## Do I need PowerShell experience to start using PSRule for CAF?
...
Absolutely.
PSRule for CAF does not replace Azure Advisor, Security Center, or Azure Policy.
...
PSRule and PSRule for CAF currently do not collect any telemetry during installation or execution.

PowerShell (used by PSRule for CAF) does collect basic telemetry by default.
