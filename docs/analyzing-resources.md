...
The current state of Azure resources can be tested with PSRule for CAF, referred to as _in-flight_ analysis.
...
```powershell
Invoke-PSRule -InputPath 'out/' -Module 'PSRule.Rules.CAF';
```
...
Invoke-PSRule -InputPath 'out/' -Module 'PSRule.Rules.CAF' -Outcome Fail;
```
...
Invoke-PSRule -InputPath 'out/' -Module 'PSRule.Rules.CAF' -As Summary;
```
...
PSRule for CAF comes with many configuration options.
